...
protected:
    HuaweiMemory *huawei_memory = new HuaweiMemory(0, MemoryType::HUAWEI_ASCEND_NPU);
...
TEST_F(HuaweiMemoryTest, CopyHostToBuffer_And_BufferToHost_HappyPath) {
    ...
    // 分配设备缓冲区
    EXPECT_EQ(huawei_memory->allocate_buffer((void**)&device_buffer, size), status_t::SUCCESS);

    // 从主机到设备
    EXPECT_EQ(huawei_memory->copy_host_to_device(device_buffer, source, size), status_t::SUCCESS);

    // 从设备到主机
    char destination[size];
    EXPECT_EQ(huawei_memory->copy_device_to_host(destination, device_buffer, size), status_t::SUCCESS);
...
TEST_F(HuaweiMemoryTest, CopyHostToBuffer_NullDest) {
    char source[] = "Test Data";
    EXPECT_EQ(huawei_memory->copy_host_to_device(nullptr, source, sizeof(source)), status_t::ERROR);
}

// 异常路径：copy_device_to_host 时源指针为空
TEST_F(HuaweiMemoryTest, CopyBufferToHost_NullSrc) {
    char destination[20] = {0};
    EXPECT_EQ(huawei_memory->copy_device_to_host(destination, nullptr, sizeof(destination)), status_t::ERROR);
}
...
TEST_F(HuaweiMemoryTest, CopyDeviceToDevice_HappyPath) {
    ...
    // 分配设备缓冲区
    EXPECT_EQ(huawei_memory->allocate_buffer(&src, bufferSize), status_t::SUCCESS);
    EXPECT_EQ(huawei_memory->allocate_buffer(&dest, bufferSize), status_t::SUCCESS);
    ...
    EXPECT_EQ(huawei_memory->copy_host_to_device(src, sourceData, bufferSize), status_t::SUCCESS);
    EXPECT_EQ(huawei_memory->copy_device_to_device(dest, src, bufferSize), status_t::SUCCESS);
...
TEST_F(HuaweiMemoryTest, CopyDeviceToDevice_NullDest) {
    ...
    EXPECT_EQ(huawei_memory->allocate_buffer(&src, bufferSize), status_t::SUCCESS);
    EXPECT_EQ(huawei_memory->copy_device_to_device(nullptr, src, bufferSize), status_t::ERROR);
...
TEST_F(HuaweiMemoryTest, CopyDeviceToDevice_NullSrc) {
    ...
    EXPECT_EQ(huawei_memory->allocate_buffer(&dest, bufferSize), status_t::SUCCESS);
    EXPECT_EQ(huawei_memory->copy_device_to_device(dest, nullptr, bufferSize), status_t::ERROR);
