...
TEST_F(DeviceMemoryTest, CopyHostToBuffer_HappyPath) {
    ...
    ASSERT_EQ(device_mem_ops->allocateBuffer((void **)&src, 20), status_t::SUCCESS);
    ASSERT_EQ(device_mem_ops->copyHostToDevice(src, source, 20), status_t::SUCCESS);
...
TEST_F(DeviceMemoryTest, CopyBufferToBuffer_NullSource) {
    ...
    ASSERT_EQ(device_mem_ops->allocateBuffer(&dest, bufferSize), status_t::SUCCESS);
    EXPECT_EQ(device_mem_ops->copyDeviceToDevice(dest, nullptr, bufferSize), status_t::ERROR);
...
TEST_F(DeviceMemoryTest, SetNewIdMemoryType_NvidiaGPUtoAMDGPU_NotSupported) {
    ...
    EXPECT_THROW(memory_init_test->setDeviceIdAndMemoryType(1, MemoryType::AMD_GPU), std::runtime_error);
}
