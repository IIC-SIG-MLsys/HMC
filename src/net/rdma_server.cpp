...
RDMAServer::RDMAServer(std::shared_ptr<ConnBuffer> buffer, std::shared_ptr<ConnManager> conn_manager) : buffer(buffer), Server(conn_manager) {}
...
status_t RDMAServer::listen(const std::string& ip, uint16_t port) {
  ...
  ret = rdma_create_id(this->cm_event_channel, &this->server_cm_id, nullptr, RDMA_PS_TCP); // 2. 创建RDMA标识               
  if (ret) {
  ...
  while (!should_exit()) {
      ...
      if (cm_event->id->route.addr.src_addr.sa_family == AF_INET) {
        ...
        if (!(inet_ntop(AF_INET, &(client_addr->sin_addr), recv_ip, INET_ADDRSTRLEN))) {
...
std::unique_ptr<Endpoint> RDMAServer::handleConnection(rdma_cm_id *id) {
  ...
  if (rdma_accept(endpoint->cm_id, &conn_param)) {
    logError("Server::server_accept_newconnection: Failed to accept connection");
    goto failed;
  }
