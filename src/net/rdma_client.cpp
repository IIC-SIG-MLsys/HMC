...
std::unique_ptr<Endpoint> RDMAClient::connect(const std::string& ip, uint16_t port) {
    ...
    auto endpoint = std::make_unique<RDMAEndpoint>(buffer->ptr, buffer->buffer_size);
    ...
    while (this->retry_count < this->max_retry_times) {
        ...
        ret = rdma_create_id(endpoint->cm_event_channel, &endpoint->cm_id, nullptr, RDMA_PS_TCP); // 创建RDMA标识id
        if (ret) {
            logError("Client::setup_client: Failed to create cm id");
            goto failed;
        }
        ret = rdma_resolve_addr(endpoint->cm_id, nullptr, (struct sockaddr *)&this->sockaddr, resolve_addr_timeout_ms); // 解析地址
        if (ret) {
            logError("Client::setup_client: Failed to resolve addr");
            goto retry;
        }
        ret = rdma_get_cm_event(endpoint->cm_event_channel, &cm_event); // 等地址解析完成
        if (ret) {
            logError("Client::setup_client: Failed to get cm event");
            goto retry;
        }
        if (cm_event->event != RDMA_CM_EVENT_ADDR_RESOLVED) {
            logError("Client::setup_client: Unexpected cm event %s", rdma_event_str(cm_event->event));
            rdma_ack_cm_event(cm_event);
            goto retry;
        }
        rdma_ack_cm_event(cm_event);
        ret = rdma_resolve_route(endpoint->cm_id, resolve_addr_timeout_ms); // 解析路由
        if (ret) {
            logError("Client::setup_client: Failed to resolve route, ret = %d", ret);
            goto retry;
        }
        logDebug("Client::setup_client: route resolved");
        ret = rdma_get_cm_event(endpoint->cm_event_channel, &cm_event); // 等待路由解析完成
        if (ret) {
            logError("Client::setup_client: Failed to get cm event");
            goto retry;
        }
        ...
        endpoint->completion_channel = ibv_create_comp_channel(endpoint->cm_id->verbs); // 创建完成通道
        if (!endpoint->completion_channel) {
            logError("Failed to create completion channel");
            goto failed;
        }
        endpoint->cq = ibv_create_cq(endpoint->cm_id->verbs, endpoint->cq_capacity, nullptr,
        ...
        memset(&endpoint->qp_init_attr, 0, sizeof(endpoint->qp_init_attr)); // 初始化QP属性
        ...
        ret = rdma_create_qp(endpoint->cm_id, endpoint->pd, &endpoint->qp_init_attr); // 创建QP
        if (ret) {
        ...
        if (rdma_get_cm_event(endpoint->cm_event_channel, &cm_event)) {
            logError("Client::start_client: Failed to get RDMA_CM_EVENT_ESTABLISHED event");
            goto retry;
        }
        ...
        // 交换元数据：buffer的元信息交换
        if (exchangeMetadata(endpoint) != status_t::SUCCESS) {// metadata for both side.
...
status_t RDMAClient::exchangeMetadata(std::unique_ptr<RDMAEndpoint>& endpoint){
    ...
    // 打印调试信息
    logDebug("Client::client_exchange_metadata: Local metadata:");
    endpoint->showRdmaBufferAttr(&endpoint->local_metadata_attr);
    logDebug("Client::client_exchange_metadata: Remote metadata:");
    endpoint->showRdmaBufferAttr(&endpoint->remote_metadata_attr);

    /** TODO：在endpoint->setupBuffers()增加，支持更多的数据buffer; 在这里完成buffer的数据信息交换 **/
