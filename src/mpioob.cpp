...
RankInfoCollection MPIOOB::collectRankInfo() {
  ...
  if (rank == 0) {
    for (int i = 1; i < world_size; ++i) {
      ...
      MPI_Recv(buffer.data(), data_size, MPI_CHAR, i, 0, MPI_COMM_WORLD,
               &status);

      RankInfo receivedRank;
      ProtoUtils::deserializeFromString(
          receivedRank, std::string(buffer.begin(), buffer.end()));
...
void MPIOOB::distributeTask() {
  if (rank == 0) {
    ...
    MPI_Recv(taskBuffer.data(), data_size, MPI_CHAR, 0, 0, MPI_COMM_WORLD,
             &status);

    ComputationGraph receivedGraph;
    ProtoUtils::deserializeFromString(
        receivedGraph, std::string(taskBuffer.begin(), taskBuffer.end()));
...
std::string MPIOOB::getLocalIP() {
  ...
  for (ifa = ifaddr; ifa != nullptr; ifa = ifa->ifa_next) {
    ...
    if (family == AF_INET || family == AF_INET6) {
      ...
      if (s != 0) {
        std::cerr << "Error: getnameinfo() failed: " << gai_strerror(s)
                  << std::endl;
        continue;
      }
